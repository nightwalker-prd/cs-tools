...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('robotics-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('robotics-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('robotics-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Bot className="w-16 h-16 text-[#607D8B] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Robotics Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Robotics concepts explorer.
