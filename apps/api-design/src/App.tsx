...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('api-design-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('api-design-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('api-design-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Plug className="w-16 h-16 text-[#F5A97F] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">API Design & Protocols Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master API design — RESTful patterns, GraphQL, gRPC, authentication, and API operations.
