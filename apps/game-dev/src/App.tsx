...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('game-dev-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('game-dev-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('game-dev-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Gamepad2 className="w-16 h-16 text-[#FF5722] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Game Development Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Game development concepts explorer.
