...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('data-science-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('data-science-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('data-science-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <FlaskConical className="w-16 h-16 text-[#DA70D6] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Data Science Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in data science — from statistics and probability to data pipelines and experimentation.
