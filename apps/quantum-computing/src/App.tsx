...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('quantum-computing-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('quantum-computing-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('quantum-computing-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Atom className="w-16 h-16 text-[#00BCD4] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Quantum Computing Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Quantum computing concepts explorer.
