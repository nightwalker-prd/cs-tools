...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('ml-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('ml-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('ml-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <BrainCircuit className="w-16 h-16 text-[#7EE787] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">ML Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in machine learning — from linear regression to transformers and reinforcement learning.
