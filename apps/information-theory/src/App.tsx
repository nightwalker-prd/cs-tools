...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('information-theory-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('information-theory-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('information-theory-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Radio className="w-16 h-16 text-[#D19A66] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Information Theory & Coding Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore information theory — entropy, compression algorithms, error-correcting codes, and Shannon limits.
