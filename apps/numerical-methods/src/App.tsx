...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('numerical-methods-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('numerical-methods-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('numerical-methods-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Sigma className="w-16 h-16 text-[#CBA6F7] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Numerical Methods & Linear Algebra Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master numerical methods — linear algebra, optimization, floating-point analysis, and numerical computing.
