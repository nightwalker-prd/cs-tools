...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('tech-ethics-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('tech-ethics-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('tech-ethics-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Scale className="w-16 h-16 text-[#795548] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Tech Ethics Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Technology ethics concepts explorer.
