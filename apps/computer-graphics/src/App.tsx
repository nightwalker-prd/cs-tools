...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('computer-graphics-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('computer-graphics-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('computer-graphics-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Monitor className="w-16 h-16 text-[#E06C75] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Computer Graphics Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master computer graphics — math foundations, rendering pipeline, ray tracing, and real-time techniques.
