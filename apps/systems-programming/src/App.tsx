...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('systems-programming-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('systems-programming-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('systems-programming-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Microchip className="w-16 h-16 text-[#73DACA] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Systems Programming Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master the low-level fundamentals — system calls, memory management, threads, IPC, sockets, debugging, and performance.
