...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('devops-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('devops-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('devops-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Rocket className="w-16 h-16 text-[#A5D6FF] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">DevOps Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key DevOps concepts — from containers and CI/CD pipelines to cloud infrastructure and observability.
