...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('self-hosting-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('self-hosting-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('self-hosting-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Server className="w-16 h-16 text-[#E67E22] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Self-Hosting Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Self-hosting concepts explorer.
