...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('hci-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('hci-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('hci-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <MousePointer className="w-16 h-16 text-[#3F51B5] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">HCI Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Human-computer interaction concepts explorer.
