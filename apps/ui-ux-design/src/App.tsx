...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('ui-ux-design-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('ui-ux-design-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('ui-ux-design-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Palette className="w-16 h-16 text-[#FF4081] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">UI/UX Design Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          UI/UX design concepts explorer.
