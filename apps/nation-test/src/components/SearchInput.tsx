...
import type { TestSearchResult } from '../hooks/useTestSearch';
    ...
    <div className="search-wrapper">
      ...
      {query.length >= 2 && (
        <div className="search-results">
          {results.length === 0 ? (
            <div className="search-empty">No tests found</div>
          ) : (
            results.map(r => (
              <button
                key={r.testType.type}
                ...
                onClick={() => {
                  onSelect(r.testType.type);
                  onClear();
                }}
              >
                <span className="search-result-title">{r.testType.name}</span>
                <span className="search-result-desc">{r.testType.description}</span>
