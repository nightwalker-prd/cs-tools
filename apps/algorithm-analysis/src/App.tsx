...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('algorithm-analysis-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('algorithm-analysis-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('algorithm-analysis-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <LineChart className="w-16 h-16 text-[#4ECDC4] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Algorithm Analysis Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master algorithm analysis — asymptotic notation, recurrence relations, master theorem, amortized analysis, and correctness proofs.
