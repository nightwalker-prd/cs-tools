...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('security-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('security-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('security-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <ShieldCheck className="w-16 h-16 text-[#F47067] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Security Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in security and cryptography — from encryption algorithms to incident response.
