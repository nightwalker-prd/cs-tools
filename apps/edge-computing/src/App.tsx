...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('edge-computing-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('edge-computing-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('edge-computing-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Router className="w-16 h-16 text-[#009688] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Edge Computing Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Edge computing concepts explorer covering IoT, edge platforms, edge AI, and industrial applications.
