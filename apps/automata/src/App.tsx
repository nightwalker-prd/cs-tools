...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('automata-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('automata-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('automata-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Infinity className="w-16 h-16 text-[#C678DD] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Automata & Formal Languages Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore automata theory — finite automata, regular languages, context-free grammars, and computability.
