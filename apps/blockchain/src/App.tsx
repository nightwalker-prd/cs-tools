...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('blockchain-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('blockchain-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('blockchain-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Link className="w-16 h-16 text-[#F7931A] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Blockchain Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Blockchain concepts explorer.
