...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('fp-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('fp-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('fp-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <FunctionSquare className="w-16 h-16 text-[#E06C75] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">FP Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in functional programming — from pure functions and immutability to monads and type systems.
