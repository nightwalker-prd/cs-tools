...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('compilers-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('compilers-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('compilers-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Terminal className="w-16 h-16 text-[#61AFEF] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Compilers & DSLs</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in compilers and domain-specific languages — from parsing and ASTs to code generation and DSL design. Select a topic from the sidebar to begin.
        </p>
