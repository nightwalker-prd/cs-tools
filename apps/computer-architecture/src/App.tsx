...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('computer-architecture-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('computer-architecture-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('computer-architecture-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <CircuitBoard className="w-16 h-16 text-[#FF6B6B] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Computer Architecture Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master the fundamentals of computer architecture — digital logic, CPU pipelines, caches, and modern processor design.
