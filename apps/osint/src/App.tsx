...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('osint-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('osint-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('osint-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Radar className="w-16 h-16 text-[#00D4AA] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">OSINT Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master OSINT fundamentals — reconnaissance, SOCMINT, GEOINT, verification, threat intelligence, and operational security.
