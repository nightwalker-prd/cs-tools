...
import { ZOOM_MIN, ZOOM_MAX, ZOOM_STEP, ZOOM_DEFAULT, NAV_IDLE_TIMEOUT } from '../config/pdf';
...
export function PdfViewer({ pdfUrl, pageKey }: PdfViewerProps) {
  const [currentPage, setCurrentPage] = usePersistedState<number>(pageKey, 1);
  ...
  // Double-buffer state
  const [pendingPage, setPendingPage] = useState<number>(currentPage);
  const [displayPage, setDisplayPage] = useState<number>(currentPage);
  const [activeSlot, setActiveSlot] = useState<'A' | 'B'>('A');
  const pendingPageRef = useRef<number>(currentPage);
  const [pageInput, setPageInput] = useState<string>(String(currentPage));

  // Slot page assignments
  const [slotAPage, setSlotAPage] = useState<number>(currentPage);
  const [slotBPage, setSlotBPage] = useState<number>(currentPage);
