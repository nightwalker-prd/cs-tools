...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('databases-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('databases-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('databases-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Database className="w-16 h-16 text-[#F778BA] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Databases Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in database systems — from storage engines to distributed transactions.
