...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('browsers-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('browsers-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('browsers-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Globe className="w-16 h-16 text-[#4285F4] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Browser Internals Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Browser internals concepts explorer.
