...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('discrete-math-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('discrete-math-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('discrete-math-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Binary className="w-16 h-16 text-[#FF9E64] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Discrete Math Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in discrete mathematics — from logic and set theory to graph theory and combinatorics.
