...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('software-engineering-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('software-engineering-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('software-engineering-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Wrench className="w-16 h-16 text-[#E5C07B] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Software Engineering Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master the discipline of building software — requirements, testing, code quality, agile practices, and production readiness.
