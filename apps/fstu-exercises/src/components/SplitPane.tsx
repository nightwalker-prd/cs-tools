...
import { PANEL_WIDTH_DEFAULT, PANEL_WIDTH_MIN, PANEL_WIDTH_MAX, PANEL_WIDTH_KEY } from '../config/pdf';
...
export function SplitPane({ pdfPane, exercisePane }: SplitPaneProps) {
  ...
  const [activeTab, setActiveTab] = useState<'textbook' | 'exercises'>('exercises');
    ...
    return (
      <div className="split-pane-mobile">
        <div className="split-tab-bar">
          ...
          <button
            className={`split-tab ${activeTab === 'exercises' ? 'active' : ''}`}
