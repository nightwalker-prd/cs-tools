...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('os-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('os-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('os-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Cpu className="w-16 h-16 text-[#56D4DD] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Operating Systems Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in operating systems — from process management to virtualization.
