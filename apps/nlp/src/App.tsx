...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('nlp-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('nlp-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('nlp-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <MessageSquareText className="w-16 h-16 text-[#9C27B0] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">NLP Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Natural language processing concepts explorer.
