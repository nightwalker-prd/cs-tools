...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('adult-tech-innovation-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('adult-tech-innovation-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('adult-tech-innovation-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Flame className="w-16 h-16 text-[#FF6B6B] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Adult Industry Tech Innovation</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore how the adult industry drove tech innovation — streaming video, online payments, CDNs, compression, privacy tech, and content moderation at scale. Select a topic from the sidebar to begin.
        </p>
