...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('web-animations-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('web-animations-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('web-animations-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Sparkles className="w-16 h-16 text-[#E040FB] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Web Animations Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Web animation concepts explorer.
