...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('reverse-engineering-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('reverse-engineering-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('reverse-engineering-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Microscope className="w-16 h-16 text-[#00CED1] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Reverse Engineering Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master reverse engineering fundamentals — binary analysis, disassembly, debugging, malware analysis, firmware RE, and exploit development.
