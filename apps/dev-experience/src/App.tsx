...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('dev-experience-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('dev-experience-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('dev-experience-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <SquareTerminal className="w-16 h-16 text-[#98C379] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Developer Experience Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master the terminal, git, scripting, and automation — the little things that make developers fast and productive.
