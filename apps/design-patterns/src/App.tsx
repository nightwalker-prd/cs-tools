...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('design-patterns-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('design-patterns-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('design-patterns-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Blocks className="w-16 h-16 text-[#E3B341] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Design Patterns Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore essential software design patterns — from creational to architectural patterns.
