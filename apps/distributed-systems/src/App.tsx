...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('distributed-systems-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('distributed-systems-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('distributed-systems-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <GitBranchPlus className="w-16 h-16 text-[#C9A0FF] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Distributed Systems Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore key concepts in distributed systems — from consensus algorithms to distributed storage and fault tolerance.
