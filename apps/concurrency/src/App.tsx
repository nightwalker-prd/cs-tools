...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('concurrency-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('concurrency-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('concurrency-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Workflow className="w-16 h-16 text-[#56B6C2] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Concurrency & Parallelism Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master concurrency and parallelism — threading models, synchronization, lock-free algorithms, and async patterns.
