...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('cryptography-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('cryptography-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('cryptography-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <KeyRound className="w-16 h-16 text-[#FFD700] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Cryptography Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master cryptography — symmetric and asymmetric ciphers, elliptic curves, TLS, post-quantum crypto, cryptanalysis, and cryptographic protocols.
