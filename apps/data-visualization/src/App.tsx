...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('data-visualization-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('data-visualization-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('data-visualization-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <PieChart className="w-16 h-16 text-[#FF9800] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Data Visualization Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Data visualization concepts explorer.
