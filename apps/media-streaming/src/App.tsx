...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('media-streaming-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('media-streaming-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('media-streaming-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <Tv className="w-16 h-16 text-[#E879F9] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Media Streaming Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Master media streaming — video codecs, streaming protocols, CDN architecture, DRM, and platform engineering patterns.
