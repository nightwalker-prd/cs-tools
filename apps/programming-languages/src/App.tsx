...
export default function App() {
  ...
  const [bookmarks, setBookmarks] = usePersistedState<string[]>('programming-languages-bookmarks', []);
  const [completedChapters, setCompletedChapters] = usePersistedState<number[]>('programming-languages-completed', []);
  const [quizScores, setQuizScores] = usePersistedState<Record<number, QuizScore>>('programming-languages-quiz-scores', {});
  ...
  const renderContent = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-20 space-y-6">
        <FileCode className="w-16 h-16 text-[#61AFEF] mx-auto" />
        <h2 className="text-2xl font-bold text-[#E6EDF3]">Programming Languages Explorer</h2>
        <p className="text-[#8B949E] leading-relaxed">
          Explore programming language theory — type systems, evaluation strategies, paradigms, and runtime design.
